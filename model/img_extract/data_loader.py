...
def load_img_features(data_dir, split):
	import h5py
	img_features = None
	img_id_list = None
	with h5py.File( join( data_dir, (split + '_img.h5')),'r') as hf:
		img_features = np.array(hf.get('img_features'))
	with h5py.File( join( data_dir, (split + '_img_id_list.h5')),'r') as hf:
		img_id_list = np.array(hf.get('img_id_list'))
	return img_features, img_id_list
